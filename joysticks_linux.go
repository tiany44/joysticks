...
func (d HID) populate() {
	for {
		...
		switch evt.Type {
		case 0x81:
			d.Buttons[evt.Index] = button{evt.Index, toDuration(evt.Time), evt.Value != 0}
		case 0x82:
			d.HatAxes[evt.Index] = hatAxis{evt.Index, evt.Index, false, toDuration(evt.Time), float32(evt.Value) / maxValue}
		default:
